...
dJ2deT = -primary.mu*primary.J2*primary.Re^2/2/a^3*(...
    0*(6*e*(3*sin(i)^2*cos(aop)^2-1)/(1-e^2)^4 + ...
    dCDende(1)*cos(f) -...
    ...
    0*dSDende(4)*sin(f).*cos(f).^4+...
    0*S(4)*(cos(f).^5-4*sin(f).^2.*cos(f).^3).*dfde/(1-e^2)^3));
...
dRBarde = 0*-3*e*(3*cos(i)^2-1)/2/(1-e^2)^(5/2); %
...
b4 = 0*[-1,8*e,-24*e^2-3,32*e^3+16*e,-16*e^4-24*e^2-2,0,16*e^4+24*e^2+2,...
...
db4de = 0*[0,8,-48*e,96*e^2+16,-64*e^3-48*e,0,64*e^3+48*e,-96*e^3-16,48*e,-8,0].';
...
while max(abs([Ak,Bk])) > tol && k < kMax
    ...
    dJ2deFreq = [dJ2deFreq R0Tilde*[0*Ak;Bk]]; %#ok<AGROW> 
    k = k+1;
end
