...
while max(abs([Ak,Bk])) > tol && k < kMax
    ...
    dg2de = abs(m2+n+k-2)*dbde.*b.^(abs(m2+n+k-2)-1);
    dg3de = abs(m3+n+k-2)*dbde.*(abs(m3+n+k-3).*b.^(abs(m3+n+k-3)-1) + ...
            e/sqrt(1-e^2)*b.^(abs(m3+n+k-2)-1)) + ...
            b.^abs(m3+n+k-2)/(1-e^2)^(3/2);
    dg4de = abs(m4+n+k-2)*dbde.*(3*e^2/2/(1-e^2)*b.^(abs(m4+n+k-2)-1) +...
            abs(m4+n+k-3).*(3*e/2/sqrt(1-e^2)*b.^(abs(m4+n+k-3)-1) +...
            abs(m4+n+k-4)/2.*b.^(abs(m4+n+k-4)-1))) +...
            3*abs(m4+n+k-2)/(1-e^2)^(3/2)/2.*b.^abs(m4+n+k-3) +...
            3*e*(1+e^2)/(1-e^2)^3*b.^abs(m4+n+k-2);
    dg5de = abs(m5+n+k-2)*dbde.*(5*e^3/2/(1-e^2)^(3/2)*b.^(abs(m5+n+k-2)-1) + ...
            abs(m5+n+k-3).*(5*e^2/2/(1-e^2)*b.^(abs(m5+n+k-3)-1) + ...
            abs(m5+n+k-4).*(e/sqrt(1-e^2)*b.^(abs(m5+n+k-4)-1) + ...
            abs(m5+n+k-5)/6.*b.^(abs(m5+n+k-5)-1)))) + ...
            abs(m5+n+k-3).*abs(m5+n+k-2)/(1-e^2)^(3/2).*b.^abs(m5+n+k-4) +...
            5*abs(m5+n+k-2)*e*(1+e^2)/(1-e^2)^3.*b.^abs(m5+n+k-3) +...
    ...
    while max(abs([dAk,dBk])) > nTol || n <= k + 5
        ...
        dBk = 0*besselj(n,-k*e)*S.'*[b1.'*g2; b2.'*g3; b3.'*g4; b4.'*g5];
        ...
        dBk = dBk + 0*besselj(-n,-k*e)*S.'*[b1.'*g2; b2.'*g3; b3.'*g4; b4.'*g5];
